...
describe("toSerializedQuery", () => {
	...
	it("captures a single selected dex id into dexNumber", () => {
		const s = baseSearch();
		s.ids = ["112"];
		expect(toSerializedQuery(s, {}).dexNumber).toBe(112);
	});

	it("a single non-dex id (trainer name) captures no dex context", () => {
		const s = baseSearch();
		s.ids = ["Barry"];
		expect(toSerializedQuery(s, {}).dexNumber).toBeNull();
	});

	it("multiple selected ids give no single dex context", () => {
		const s = baseSearch();
		s.ids = ["112", "6"];
		expect(toSerializedQuery(s, {}).dexNumber).toBeNull();
	});

	it("dex context wins over the ids filter", () => {
		const s = baseSearch();
		s.ids = ["112"];
	...
	it("passes mode through (default 'fuzzy')", () => {
		expect(toSerializedQuery(baseSearch(), baseCtx()).mode).toBe("fuzzy");
		const s = baseSearch();
		s.mode = "exact";
		expect(toSerializedQuery(s, baseCtx()).mode).toBe("exact");
		s.mode = "contains";
		expect(toSerializedQuery(s, baseCtx()).mode).toBe("contains");
	});
});
...
describe("isRuleCapturable", () => {
	...
	it("mode alone is NOT a capturable constraint", () => {
		// mode only changes how text matches; with no text/filters it matches nothing useful
		expect(isRuleCapturable({ ...empty(), mode: "exact" })).toBe(false);
	});
