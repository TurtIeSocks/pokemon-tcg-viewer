...
- **The service worker is hand-rolled, not `vite-plugin-pwa`.** That plugin's SW/manifest generation does **not** run with TanStack Start (its `closeBundle`/`generateSW` hooks never fire — TanStack/router#4988). The dep was removed. `public/sw.js` is a plain runtime-cache SW registered lazily in `__root.tsx` via `registerBrowseCacheSW`.
- **SW caches, by concern:** wsrv.nl card images (thumbs/hires, capped, settings UI) **and** the app shell — NetworkFirst navigations → `/offline.html` fallback, CacheFirst for content-hashed JS/CSS/fonts (immutable names = stale-safe), StaleWhileRevalidate same-origin images. RPCs (`/_serverFn`), `/api`, `/corpus`, and cross-origin pass through. **Asset caching is skipped on `localhost`** so it never breaks Vite HMR. The routing policy is a tested pure fn `src/store/offline-images/sw-shell-policy.ts` **mirrored** in `sw.js` (which can't import from src — keep them in sync, same pattern as `cache-policy.ts`).
- **`offline_indicator` is localized in all 17 UI locales.** New root-head/manifest strings must stay parity-complete or `src/lib/i18n-parity.test.ts` fails (16 non-en fails). Reuse an existing translation (e.g. `sync_status_offline`) when the word matches.
- **Not offline yet (gap #3):** server-fn route loaders (`getSetCardsFn`, `getCardForRouteFn`, search, pokedex) RPC on client nav, so a *first* offline navigation to an unvisited set/card/search route shows `/offline.html`. Home/Vault/card-modal + previously-visited URLs work offline. Converting those loaders to an IDB-corpus client fallback is the documented follow-up.
