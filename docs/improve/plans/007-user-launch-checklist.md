...
- [ ] **D.1 GitHub deploy key for the plugin**: generate an SSH keypair; add the public half as a read-only deploy key on the private `card-stack-cloud` repo; add the private half as Actions secret `CLOUD_DEPLOY_KEY` on the **public** repo. Set repo variable `DEPLOY_BILLING=1` when ready to ship billing (006 Task 1 gates on it).
