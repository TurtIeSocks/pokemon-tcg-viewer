...
- [ ] **D.2 `/etc/tcg/env` on the home server** — add (names only listed here; 006 Task 8's DEPLOY.md table is the authoritative inventory): `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (B.1), `STRIPE_SECRET_KEY` (C.6), `STRIPE_WEBHOOK_SECRET` (C.2), `STRIPE_PRICE_PLUS_MONTHLY`, `STRIPE_PRICE_PLUS_ANNUAL` (C.1), `APP_ORIGIN` (A.1), plus `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` (B.1 again; the SSR server reads these two at runtime as well, not just at client build time). Also add `ANTHROPIC_API_KEY` (secret, gates the Plus AI card scan at `/api/scan`; absent = that route 503s, the free on-device scanner is unaffected) and optionally `SCAN_MODEL` (default `claude-haiku-4-5`). Then `sudo systemctl restart tcg` and confirm `curl -s localhost:3000/api/health` shows `billingEnv:7` and `supabase:true`.
